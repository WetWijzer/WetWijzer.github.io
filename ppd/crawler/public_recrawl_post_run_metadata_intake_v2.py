uo"""Fixture-first public recrawl post-run metadata intake v2 packets."""
...
def build_public_recrawl_post_run_metadata_intake_v2_packet(
    ...
    *,
    ...
    """Build a metadata-only post-run intake packet from fixture inputs."""

    operator_issues = validate_public_recrawl_operator_packet_v2(operator_packet_v2)
    if operator_issues:
        issue_codes = ", ".join(issue.code for issue in operator_issues)
        raise PublicRecrawlPostRunMetadataIntakeV2PacketError(
            "operator packet v2 is not safe: " + issue_codes
        )
...
def validate_public_recrawl_post_run_metadata_intake_v2_packet(
    ...
    if not isinstance(summary, Mapping):
        ...
        for key in ("metadata_only",):
            if summary.get(key) is not True:
                errors.append("intake_summary." + key + " must be true")
        for key in ("live_crawl_performed", "processor_invoked", "raw_body_persisted", "document_downloaded", "source_registry_mutated"):
            if summary.get(key) is not False:
                errors.append("intake_summary." + key + " must be false")

    return PublicRecrawlPostRunMetadataIntakeV2ValidationResult(
        valid=not errors,
...
def _capture_records_by_key(intake_fixture: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    ...
    for capture in _sequence(intake_fixture.get("synthetic_metadata_only_capture_records")):
        if isinstance(capture, Mapping):
            source_id = _text(capture.get("source_id"))
            canonical_url = _text(capture.get("canonical_url"))
            requested_url = _text(capture.get("requested_url"))
            for key in (source_id, canonical_url, requested_url):
                if key:
                    rows[key] = capture
    return rows
